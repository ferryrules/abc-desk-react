...
(function(){
  ...
  function $(k,p){
    var D=!1;0>k&&(D=!0,k*=-1);k=""+k;for(p=p?p:1;k.length<p;)k="0"+k;return D?"-"+k:k
  }
  function Ia(k){
    if(!k)return k;k=k.replace(/^\s\s*/,"");for(var p=/\s/,D=k.length;p.test(k.charAt(--D)););return k.slice(0,D+1)
  }
  function Ea(k){
    k.roundRect=function(k,D,r,u,H,F,z,v) {
      ...
      this.lineWidth=F;this.beginPath();
      this.moveTo(k+H,D);
      this.lineTo(k+r-H,D);
      ...
      this.lineTo(k+H,D+u);
      this.quadraticCurveTo(k,D+u,k,D+u-H);this.lineTo(k,D+H);
  ...
  function Ta(k,p,D){
    if(k&&p&&D){
      ...
      if("undefined"!==typeof Blob&&new Blob){
        for(var F=k.replace(/^data:[a-z\/]*;base64,/,""),F=atob(F),z=new ArrayBuffer(F.length),z=new Uint8Array(z),v=0;
        v<F.length;v++)z[v]=F.charCodeAt(v);
        p=new Blob([z.buffer],{
          type:"image/"+p
        });
        try{
      ...
      if(!u)try{
        ...
        p=window.open(),p.document.write("<img alt='k' src='"+k+"'></img><div>Please right click on the image and save it to your device</div>"),p.document.close()
      }
    }
  }
  function N(k){
  ...
  function Za(k,p,D){
    ...
    if(isNaN(u)){
      try{
        ...
        if(!xa){
          var H=document.body;
          xa=document.createElement("span");xa.innerHTML="";
          H.appendChild(xa)
        }
        xa.style.display="";
  ...
  function R(k,p){
    var D=[];
    if(D={
      solid:[],shortDash:[3,1],shortDot:[1,1],shortDashDot:[3,1,1,1],shortDashDotDot:[3,1,1,1,1,1],dot:[1,2],dash:[4,2],dashDot:[4,2,1,2],longDash:[8,2],longDashDot:[8,2,1,2],longDashDotDot:[8,2,1,2,1,2]
    }[k||"solid"])for(var r=0;r<D.length;r++)D[r]*=p;else D=[];return D
  }
  ...
  function ab(k,p,D){
    ...
    p=!0;
    for(D=0;4>D;D++)if(k[D]!==k[D+4]|k[D]!==k[D+8]|k[D]!==k[D+12]){p=!1;break}return p?k[0]<<16|k[1]<<8|k[2]:0
  }
  function na(k,p,D){
  ...
  function V(k,p,D,r,v){
    ...
    this.themeOptions=k;this.options=D?D:{_isPlaceholder:!0};
  ...
  function Ka(k,p,r){
    ...
    "undefined"===typeof this.options.fontSize&&(this.fontSize=this.chart.getAutoFontSize(this.fontSize));this.height=this.width=null;
    this.bounds={
      x1:null,
      y1:null,
      x2:null,
...
function p(a,d){
  ...
  this._indexLabels=[];
  this._panTimerId=0;this._lastTouchEventType="";
...
p.prototype.renderElements=function(){
  ...
  this.ctx.miterLimit=a;this.animatedRender&&this._breaksCanvasCtx&&b.push({source:this._breaksCanvasCtx,
...
0;d<a.length;d++){var b=a[d];ia.drawMarker(b.x,b.y,b.ctx,b.type,b.size,b.color,b.borderColor,b.borderThickness)}}};return p}();Na.Chart.version="v2 GA"})();
